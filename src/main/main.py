...
def printCompetitionProblemSheet():
    out = open("/Users/cravuri/Documents/harker/LSHacks/src/AMC.out", "w")
    for i in range(1, 26):
        prob = getRandomProblem()
        out.write("Problem " + str(i) + ". " + normalize(prob[0]))
        for _ in range(10):
            out.write("\n")
        out.write("Answer: " + "\n" + "\n")
        answerKey.append(str(prob[1]))

def printAlg1ProblemSheet():
    out = open("/Users/cravuri/Documents/harker/LSHacks/src/AMC.out", "w")
    for i in range(1, 16):
        prob = generateRandomProblem()
        ...
        if type == 0:
            ...
            out.write("Answer: " + "\n" + "\n")
            if (len(prob) == 3):
                answerKey.append(prob[1])
            else:
                answerKey.append(prob[1] + ", " + prob[2])
        if type == 1:
        ...
        if type == 2:
            if (len(prob) == 5):
                ...
                for _ in range(10):
                    out.write("\n")
                out.write("Answer: " + "\n" + "\n")
                answerKey.append(str(prob[2]) + ", " + str(prob[3]))
            if (len(prob) == 7):
                ...
                for _ in range(10):
                    out.write("\n")
                out.write("Answer: " + "\n" + "\n")
                answerKey.append(str(prob[3]) + ", " + str(prob[4]) + ", " + str(prob[5]))

printAlg1ProblemSheet()